...
class IBVS(object):
    ...
    def set_goal(self, features, depth, diff):
        ...
        for feature in features:
            if self._L is None:
                ...
                self._L = np.vstack(
                    (self._L, self._get_jacobian(feature, depth, diff)))
        pnts = self.feature_set_to_pnts(features, depth) + diff
        self._goal = self.pnts_to_feature_set(pnts)
    ...
    def feature_set_to_pnts(self, features, depth):
        pnts = np.array([self.feature_to_pnt(np.array(f), depth)
                        for f in features])
        return pnts

    def pnts_to_feature_set(self, pnts):
        features = np.array([self.pnt_to_feature(np.array(pnt))
                            for pnt in pnts])
        return features
